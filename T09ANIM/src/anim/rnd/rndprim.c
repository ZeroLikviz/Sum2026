...
VOID TM5_RndPrimCalculateNormals( tm5PRIM *Primitive )
{
  ...
  for (i = 0; i < Primitive->NumOfV; i++)
    Primitive->Vertices[i].Normal = VecSet3(0, 0, 0);

  for (i = 0; i < Primitive->NumOfI; i += 3)
  {
    VEC Vec1 = Primitive->Vertices[Primitive->Indexes[i + 0]].Vec;
    VEC Vec2 = Primitive->Vertices[Primitive->Indexes[i + 1]].Vec;
    VEC Vec3 = Primitive->Vertices[Primitive->Indexes[i + 2]].Vec;
    VEC Cross = VecCross(VecSubVec(Vec1, Vec2), VecSubVec(Vec2, Vec3));
    Cross = VecNormalize(Cross);

    Primitive->Vertices[Primitive->Indexes[i + 0]].Normal = VecAddVec(Cross, Primitive->Vertices[Primitive->Indexes[i + 0]].Normal);
    Primitive->Vertices[Primitive->Indexes[i + 1]].Normal = VecAddVec(Cross, Primitive->Vertices[Primitive->Indexes[i + 1]].Normal);
    Primitive->Vertices[Primitive->Indexes[i + 2]].Normal = VecAddVec(Cross, Primitive->Vertices[Primitive->Indexes[i + 2]].Normal);
  }

  for (i = 0; i < Primitive->NumOfV; i++)
    Primitive->Vertices[i].Normal = VecNormalize(Primitive->Vertices[i].Normal);
}
...
VOID TM5_RndPrimApplySun( tm5PRIM *Primitive, VEC SunPos )
{
  ...
  for (i = 0; i < Primitive->NumOfV; i++)
  {
    FLT Factor = (VecDot(VecScale(SunPos, -1), Primitive->Vertices[Primitive->Indexes[i]].Normal) + 1) / 2;
    if (Factor < 0.5)
      Factor = 0.5;
    Primitive->Vertices[Primitive->Indexes[i]].Color = Vec3to4(VecScale(VecSet3(1.0, 0.7, 0.1), Factor));
  }
}
...
VOID TM5_RndPrimScale( tm5PRIM *Primitive, FLT Factor )
{
  ...
  for (i = 0; i < Primitive->NumOfV; i++)
    Primitive->Vertices[i].Vec = VecScale(Primitive->Vertices[i].Vec, Factor);
}
