...
static VOID ProcessKeyboard( tm5ANIM *Anim )
{
  ...
  /* Movement Keys */
  if (Anim->Keys['W'])
    CamPos = VecAddVec3(CamPos, VecMulNum3(CamDir,  CamSpeed));
  if (Anim->Keys['S'])
    CamPos = VecAddVec3(CamPos, VecMulNum3(CamDir, -CamSpeed));
  if (Anim->Keys['D'])
    CamPos = VecAddVec3(CamPos, VecMulNum3(VecCross3(CamDir, VecUp),  CamSpeed));
  if (Anim->Keys['A'])
    CamPos = VecAddVec3(CamPos, VecMulNum3(VecCross3(CamDir, VecUp), -CamSpeed));
...
static VOID ProcessMouse( tm5ANIM *Anim )
{
  ...
  if (Anim->Keys[VK_RBUTTON])
  {
    ...
    Right = VecCross3(CamDir, VecUp);
    CamDir = VecRotateVec3(Right, CamDir, -(NullMousePos.y - MousePos.y) * Sensitivity);
